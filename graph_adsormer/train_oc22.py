...
def train(train_loader, model,global_step, criterion, optimizer, epoch):
    ...
    for i, data in enumerate(train_loader,0):
        ...
        outputs = model(inputs,edge,targets)
...
def validation(val_loader , model, global_step,criterion):
    ...
    for i, data in enumerate(val_loader,0):
        ...
        pred = model(inputs,edge, targets)
...
class make_dataset(Dataset):
    def __init__(self, root_dir, dmin = 0, step = 0.2, random_seed = 123):
        ...
        target_file = os.path.join(self.root_dir, 'adslab_df.csv')
        target_df = pd.read_csv(f'{self.root_dir}adslab_df.csv')
        target_df = target_df[(target_df['nads']==1)& (target_df['ads_symbols']=='O')][['traj_id', 'mean_ads_e']]
        target_df = target_df[abs(target_df['mean_ads_e']) <  5 ]
        target_df.columns = ['name','target']
    ...
    def __getitem__(self, idx):
        ...
        atoms = read(f'{self.root_dir}tagged_trajs/{traj_id}.traj')
        feature_data = get_atom_property(atoms)
        edge = get_edge_feature(atoms)
        
        
        return feature_data,edge, target
...
def main():
    ...
    parser.add_argument('--feature_nums', type=int, default=11)
    ...
    dataset= make_dataset(root_dir= '/home/cut6089/research/OC22/')
    ...
    from Graph_adsormer import Transformer
